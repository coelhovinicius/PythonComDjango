...
def ver_empresa(request,id):
    ...
    # ToDo: Listar as métricas dinamicamente - Semelhante a como listar os documentos
...
def realizar_proposta(request, id):
    ...
    if total + float(percentual) > empresa.percentual_equity:
        messages.add_message(request, constants.WARNING, 'O percentual solicitado está acima do máximo permitido.')
        return redirect(f'/investidores/ver_empresa/{id}')
    
    valuation = 100 * int(valor) / int(percentual)

    if valuation <(int(empresa.valuation / 2)):
        messages.add_message(request, constants.WARNING, f'O Valuation proposto (R${valuation}) está abaixo do mínimo de R${empresa.valuation}.')
        return redirect(f'/investidores/ver_empresa/{id}')
...
def assinar_contrato(request, id):
    ...
    if request.method == 'GET':
        ...
        pi.status = 'PE'
        pi.save()
        messages.add_message(request, constants.SUCCESS, f'Contrato assinado com sucesso! Sua proposta foi enviada à empresa.')
